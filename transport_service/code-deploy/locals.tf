locals {
  ...
  ecs_dimensions_map = {
    ClusterName = data.terraform_remote_state.transport-service_infra_state.outputs.ecs_cluster_name
    ServiceName = module.transport-api-ecs-service.ecs_service
  }
}
...
locals {
  container_port  = 8081
  container_image = "prasanna1994/transport-api:${var.tag}"
  aws_account_id  = data.aws_caller_identity.current.account_id
  container_secrets = [
    {
      name      = "NAME"
      valueFrom = "arn:aws:ssm:${var.region}:${local.aws_account_id}:parameter/app/transport-api/NAME"
    },
    {
      name      = "VPCID"
      valueFrom = "arn:aws:ssm:${var.region}:${local.aws_account_id}:parameter/app/transport-api/VPCID"
    }
  ]
...
locals {
  ...
  ecs_load_balancers = [{
    ...
    target_group_arn = data.terraform_remote_state.transport-service_infra_state.outputs.alb_targetgroups_arns
